...
function ProgressBar({ completed, total }: { completed: number; total: number }) {
  ...
  return (
    <div className="w-full sm:w-[200px] h-2 rounded-full bg-[var(--portal-accent2)] overflow-hidden">
      <div className="h-full bg-[var(--mui-palette-success-main)] rounded-full" style={{ width: `${pct}%` }} />
    </div>
  )
}
...
function TaskCard({ task }: { task: ChampionTaskItem }) {
  ...
  return (
    <div
      className="bg-[var(--mui-palette-background-paper)] border border-[var(--mui-palette-divider)] rounded-xl shadow-[0px_1px_3px_rgba(16,24,40,0.07),0px_1px_3.5px_rgba(16,24,40,0.04)] p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full cursor-pointer hover:shadow-[0px_4px_8px_rgba(16,24,40,0.08),0px_2px_4px_rgba(16,24,40,0.04)] transition-shadow"
    ...
    >
      <div className="flex items-center gap-4 flex-1 min-w-0 w-full">
        ...
        <p className="text-[var(--mui-palette-text-secondary)] text-sm leading-[1.43] shrink-0 sm:hidden">{task.dueDate}</p>
      </div>

      <div className="flex flex-col items-start sm:items-end gap-2 shrink-0 w-full sm:w-auto">
        <p className="text-[var(--mui-palette-text-secondary)] text-sm leading-[1.43] hidden sm:block">{task.dueDate}</p>
...
export default function ChampionTasks() {
  ...
  return (
    <Layout>
      <div className="px-4 py-4 sm:px-6 sm:py-6 w-full flex flex-col gap-6">
        <div className="flex flex-col gap-1.5">
          <h1 className="text-[var(--mui-palette-text-primary)] font-bold text-2xl sm:text-[34px] leading-[1.235] tracking-[0.25px]">
            Champion Tasks
          </h1>
